...
- [ ] \[2025.03] 正处在毕业季...结束就有时间了。我正在打算完全弃用 RQ，转而使用更容易客制化的 SWR 来重写缓存和数据库部分的连接。另外对于一些弹出式窗口可能考虑直接用 React Portal 重写。然后还有些性能问题，比如现在暂定的主页轮播，可能也会换掉。Motion 的 Bug 也需要修一下，不好用的话也换掉吧，感觉没啥优势。另外一些 Deps 需要更新，Tailwind 升 v4，React 升 19 + Memo。一些 Deps 也需要被替换，Eslint + Prettier 考虑换 Biome。另外我其实比较关心性能问题，还在思考 electron 的可行性，虽然最近写了一些 Tauri，但其在跨平台一致性上还是不如 electron，如果是 RN 呢？可以提供类原生的 list 性能吗？之后去探索一下...
