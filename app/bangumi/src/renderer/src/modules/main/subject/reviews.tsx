    ...
    <section className="@container flex min-w-0 flex-col gap-2">
      ...
      <ul className="divide-y border-y">
        {reviews.map(({ entry, user }) => (
          <li
            className="hover:bg-accent/40 grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-baseline gap-x-4 gap-y-1 px-2 py-2.5 transition-colors @xl:grid-cols-[minmax(0,1fr)_7rem_4rem]"
            key={entry.id}
          >
            <div className="min-w-0">
              <h3 className="text-sm font-medium">
                <MyLink
                  ...
                  className="focus-visible:ring-ring block truncate rounded-sm hover:underline focus-visible:ring-2 focus-visible:outline-none"
                  title={entry.title}
                  to={`/blog/${entry.id}`}
                >
                  {entry.title}
              ...
              {entry.summary && (
                <div className="bbcode text-muted-foreground mt-1 line-clamp-1 text-xs [&_p]:inline">
